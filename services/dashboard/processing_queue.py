...
class ConnectedClient:
    ...
    def send_message(self, json: dict, should_close: bool = False, loop: asyncio.AbstractEventLoop = asyncio.get_event_loop()):        
        asyncio.run_coroutine_threadsafe(self.client.send_json(json), loop)
        if should_close:
...
class ProcessingQueue:
    logger: Logger
    general_observers: list[ConnectedClient] = []
    recording_observers: dict[str, ConnectedClient] = {}
    ...
    def perform_task(self, recording: PendingRecording, abort_signal: threading.Event):
        ...
        def update_recording_observers(progress: float, status: str):
            ...
            if (recording_observer is None): return
            
            print(f"updating recording observer, progress: {progress}, status: {status}")
            recording_observer.send_message({"type": "progress", "data": self.current_processing.dict()})
        ...
        try:
            ...
            self.send_update_to_client(100, "cancelled")
            return

    def watch(self, client: ConnectedClient):
        self.general_observers.append(client)
        self.update_general_observers()
        print(f"Added general observer. Total observers: {len(self.general_observers)}")
        

    def watch_recording(self, recording_id: str, client: ConnectedClient):
        ...
        if (self.current_processing is not None and self.current_processing.recording_id == recording_id):
            client.send_message({"type": "progress", "data": self.current_processing.dict()})
    ...
    def remove_general_observer(self, client: ConnectedClient):
        self.general_observers.remove(client)
        
    def remove_recording_observer(self, recording_id: str):
        self.recording_observers.pop(recording_id)
